...
bool COutput::MonitorTimeConvergence(CConfig *config, unsigned long TimeIteration) {
  ...
  if(Inner_IterConv && TimeIteration >= config->GetStartWindowIteration()){
     ...
     if(GetHistoryFields().count(WndConv_Field) > 0 ){
       ...
       if (GetHistoryFields()[WndConv_Field].fieldType == HistoryFieldType::COEFFICIENT) {
       ...
       if (GetHistoryFields()[WndConv_Field].fieldType == HistoryFieldType::RESIDUAL || GetHistoryFields()[WndConv_Field].fieldType == HistoryFieldType::AUTO_COEFFICIENT) {
...
void COutput::Postprocess_HistoryFields(CConfig *config){
  ...
  if (config->GetTime_Domain()){
    for (unsigned short iField = 0; iField < historyOutput_List.size(); iField++){
        ...
        const HistoryOutputField &currentField = historyOutput_Map[fieldIdentifier];
      if (currentField.fieldType == HistoryFieldType::COEFFICIENT){
        AddHistoryOutput("TAVG_"   + historyOutput_List[iField], "tavg["  + currentField.fieldName + "]", currentField.screenFormat, "TAVG_"   + currentField.outputGroup, "Time averaged values.", HistoryFieldType::AUTO_COEFFICIENT);
        AddHistoryOutput("SQ_WND_AVG_" + historyOutput_List[iField]       , "sq_wnd_avg[" + currentField.fieldName + "]"      , currentField.screenFormat, "SQ_WND_AVG_" + currentField.outputGroup, "Time averaged square window weighted values.", HistoryFieldType::AUTO_COEFFICIENT);
        AddHistoryOutput("HANN_WND_AVG_" + historyOutput_List[iField]     , "hann_wnd_avg[" + currentField.fieldName + "]"    , currentField.screenFormat, "HANN_WND_AVG_" + currentField.outputGroup, "Time averaged hann window weighted values.", HistoryFieldType::AUTO_COEFFICIENT);
        AddHistoryOutput("HANNSQ_WND_AVG_" + historyOutput_List[iField]   , "hannSq_wnd_avg[" + currentField.fieldName + "]"  , currentField.screenFormat, "HANNSQ_WND_AVG_" + currentField.outputGroup, "Time averaged hann-square window weighted values.", HistoryFieldType::AUTO_COEFFICIENT);
        AddHistoryOutput("BUMP_WND_AVG_" + historyOutput_List[iField]     , "bump_wnd_avg[" + currentField.fieldName + "]"    , currentField.screenFormat, "BUMP_WND_AVG_" + currentField.outputGroup, "Time averaged bump window weighted values.", HistoryFieldType::AUTO_COEFFICIENT);
      }
    }
  }
 ...
 if (config->GetTime_Domain() && config->GetDirectDiff()){
   for (unsigned short iField = 0; iField < historyOutput_List.size(); iField++){
       ...
       const HistoryOutputField &currentField = historyOutput_Map[fieldIdentifier];
     if (currentField.fieldType == HistoryFieldType::COEFFICIENT){
       AddHistoryOutput("D_TAVG_" + historyOutput_List[iField], "dtavg[" + currentField.fieldName + "]", currentField.screenFormat, "D_TAVG_" + currentField.outputGroup, "Derivative of the time averaged value (DIRECT_DIFF=YES).", HistoryFieldType::AUTO_COEFFICIENT);
       AddHistoryOutput("D_SQ_WND_AVG_" + historyOutput_List[iField]     , "d_sq_wnd_avg[" + currentField.fieldName + "]"    , currentField.screenFormat, "D_SQ_WND_AVG_" + currentField.outputGroup, "Time averaged square window weighted values (DIRECT_DIFF=YES).", HistoryFieldType::AUTO_COEFFICIENT);
       AddHistoryOutput("D_HANN_WND_AVG_" + historyOutput_List[iField]   , "d_hann_wnd_avg[" + currentField.fieldName + "]"  , currentField.screenFormat, "D_HANN_WND_AVG_" + currentField.outputGroup, "Time averaged hann window weighted values (DIRECT_DIFF=YES).", HistoryFieldType::AUTO_COEFFICIENT);
       AddHistoryOutput("D_HANNSQ_WND_AVG_" + historyOutput_List[iField] , "d_hannsq_wnd_avg[" + currentField.fieldName + "]", currentField.screenFormat, "D_HANNSQ_WND_AVG_" + currentField.outputGroup, "Time averaged hann-square window weighted values (DIRECT_DIFF=YES).", HistoryFieldType::AUTO_COEFFICIENT);
       AddHistoryOutput("D_BUMP_WND_AVG_" + historyOutput_List[iField]   , "d_bump_wnd_avg[" + currentField.fieldName + "]"  , currentField.screenFormat, "D_BUMP_WND_AVG_" + currentField.outputGroup, "Time averaged bump window weighted values (DIRECT_DIFF=YES).", HistoryFieldType::AUTO_COEFFICIENT);
     }
   }
 }
 for (unsigned short iFieldConv = 0; iFieldConv < convFields.size(); iFieldConv++){
   ...
   if (historyOutput_Map.count(convField) > 0){
    if (historyOutput_Map[convField].fieldType == HistoryFieldType::COEFFICIENT){
        AddHistoryOutput("CAUCHY", "C["  + historyOutput_Map[convField].fieldName + "]", ScreenOutputFormat::SCIENTIFIC, "CAUCHY","Cauchy residual value of field set with CONV_FIELD." ,HistoryFieldType::AUTO_COEFFICIENT);
    }
    if(!config->GetWndConv_Field().empty()){
        AddHistoryOutput("TIME_WND_CAUCHY", "t_wnd_C[" + historyOutput_Map[config->GetWndConv_Field()].fieldName+ "]", ScreenOutputFormat::SCIENTIFIC, "TIME_WND_CAUCHY", "Cauchy residual value of field set with WND_CONV_FIELD.", HistoryFieldType::AUTO_COEFFICIENT);
    }
   }
  }
}
